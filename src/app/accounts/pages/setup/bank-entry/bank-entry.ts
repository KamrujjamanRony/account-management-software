...
@Component({
  ...
  templateUrl: './bank-entry.html',
  styleUrl: './bank-entry.css'
})
export class BankEntryComponent {
  ...
  onLoadBanks() {
    const { data$, isLoading$, hasError$ } = this.dataFetchService.fetchData(this.bankService.search(""));
  ...
  onSubmit(e: Event) {
    ...
    if (this.form.valid) {
      ...
      if (this.selectedBank) {
        ...
        this.bankService.add(this.form.value)
          .subscribe({
            next: (response) => {
              if (response !== null && response !== undefined) {
