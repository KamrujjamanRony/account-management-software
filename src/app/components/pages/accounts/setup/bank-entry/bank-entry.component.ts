        ...
        combineLatest([
          ...
          map(([data, query]) =>
            data.filter((bankData: any) =>
              ...
              bankData.address?.includes(query) ||
              bankData.remarks?.includes(query)
            )
          )
        ).subscribe(filteredData => this.filteredBankList.set(filteredData));
      }
          ...
          if(confirm("Are you sure you want to delete?")) {
            this.bankService.deleteBank(id).subscribe(data => {
              if (data.id) {
                ...
                console.error('Error deleting Bank fee:', data);
              }
            });
          }
        }
